...
complete -c date -l debug -d 'annotate the parsed date, and warn about questionable usage to standard error'
complete -c date -s f -l file -d 'like --date; once for each line of DATEFILE; if DATEFILE is -, read names fro…'
