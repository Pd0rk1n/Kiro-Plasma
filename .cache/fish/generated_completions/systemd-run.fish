...
complete -c systemd-run -l root-directory -d 'Runs the service process with the specified root directory'
complete -c systemd-run -l same-root-dir -s R -d 'Similar to --root-directory=, but uses the root directory of the systemd-run …'
...
complete -c systemd-run -l pty-late -s T -d 'Much like --pty, but the PTY communication only begins once the unit start is…'
...
complete -c systemd-run -s v -l verbose -d 'Display unit log output while running. sp Added in version 258'
complete -c systemd-run -l output -d 'Controls the formatting of verbose unit log output, see journalctl(1) for pos…'
...
complete -c systemd-run -l job-mode -d 'When queuing a new job, this option controls how to deal with already queued …'
...
complete -c systemd-run -l no-pager -d 'Do not pipe output into a pager.  This currently only applies to --help'
