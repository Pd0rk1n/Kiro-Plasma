...
complete -c nmcli -l permanent -d 'option to persist the managed state to disk, and not only in runtime.  With'
complete -c nmcli -l permanent-only -d 'only the permanent managed state is set, and not the runtime managed state'
