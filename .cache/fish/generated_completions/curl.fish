...
complete -c curl -l remote-name -d 'command line, it similarly needs multiple options for where to save them'
complete -c curl -l silent -d 'VERSION This man page describes curl 8. 21. 0'
complete -c curl -l abstract-unix-socket -d '(HTTP) Connect to the server through an abstract Unix domain socket, instead …'
...
complete -c curl -l ca-native -d '(TLS) Use the operating system\'s native CA store for certificate verification'
...
complete -c curl -l ciphers -d '(TLS) Specify which cipher suites to use in the connection if it negotiates T…'
complete -c curl -l compressed -d '(HTTP) Request a compressed response using one of the algorithms curl support…'
complete -c curl -l compressed-ssh -d '(SCP SFTP) Enable SSH compression'
complete -c curl -s K -l config -d 'Specify a text file to read curl arguments from'
complete -c curl -s O -d 'referer = "http://nowhereatall. example'
...
complete -c curl -l create-dirs -d 'When used in conjunction with the --output option, curl creates the necessary…'
...
complete -c curl -s d -l data -d '(HTTP MQTT) Send the specified data in a POST request to the HTTP server, in …'
...
complete -c curl -l data-ascii -d '(HTTP) This option is an alias for --data'
complete -c curl -s H
...
complete -c curl -l doh-cert-status -d '(DNS) Same as --cert-status but used for DoH (DNS-over-HTTPS)'
complete -c curl -l doh-insecure -d '(DNS) By default, every connection curl makes to a DoH server is verified to …'
complete -c curl -l doh-url -d '(DNS) Specify which DNS-over-HTTPS (DoH) server to use to resolve hostnames, …'
...
complete -c curl -l ech -d '(HTTPS) Specify how to do ECH (Encrypted Client Hello)'
...
complete -c curl -l false-start -d '(TLS) No TLS backend currently supports this feature'
complete -c curl -l follow -d '(HTTP) Instructs curl to follow HTTP redirects and to do the custom request m…'
...
complete -c curl -s P -l ftp-port -d '(FTP) Reverse the default initiator/listener roles when connecting with FTP'
...
complete -c curl -l ftp-ssl-ccc-mode -d '(FTP) Set the CCC mode'
complete -c curl -l ftp-ssl-control -d '(FTP) Require SSL/TLS for the FTP login, clear for transfer'
complete -c curl -s G -l get -d '(HTTP) When used, this option makes all data specified with --data, --data-bi…'
complete -c curl -s g -l globoff -d 'Switch off the URL globbing function'
complete -c curl -l happy-eyeballs-timeout-ms -d 'Set the timeout for Happy Eyeballs'
complete -c curl -l haproxy-clientip -d '(HTTP) Set a client IP in HAProxy PROXY protocol v1 header at the beginning o…'
...
complete -c curl -l http2-prior-knowledge -d '(HTTP) Issue a non-TLS HTTP request using HTTP/2 directly without HTTP/1'
complete -c curl -l http3 -d '(HTTP) Attempt HTTP/3 to the host in the URL, but fallback to earlier HTTP ve…'
complete -c curl -l http3-only -d '(HTTP) Instruct curl to use HTTP/3 to the host in the URL, with no fallback t…'
...
complete -c curl -l ip-tos -d 'Set Type of Service (TOS) for IPv4 or Traffic Class for IPv6'
complete -c curl -l ipfs-gateway -d '(IPFS) Specify which gateway to use for IPFS and IPNS URLs'
complete -c curl -s 4 -l ipv4 -d 'Request only IPv4 addresses when resolving hostnames, and not for example any…'
complete -c curl -s 6 -l ipv6 -d 'Request only IPv6 addresses when resolving hostnames, and not for example any…'
complete -c curl -l json -d '(HTTP) Send the specified JSON data in a POST request to the HTTP server'
...
complete -c curl -l key -d '(TLS SCP SFTP) Private key filename'
complete -c curl -l key-type -d '(TLS) Private key file type'
complete -c curl -l knownhosts -d '(SCP SFTP) When doing SCP and SFTP transfers, curl automatically checks a dat…'
complete -c curl -l krb -d '(FTP) Deprecated option (added in 8. 17. 0).  It has no function anymore'
...
complete -c curl -l show-headers
complete -c curl -l oauth2-bearer -d 'its credentials to the initial host'
complete -c curl -l location-trusted -d '(HTTP) Instruct curl to follow HTTP redirects like --location, but permit cur…'
...
complete -c curl -l max-redirs -d '(HTTP) Set the maximum number of redirections to follow'
complete -c curl -s m -l max-time -d 'Set the maximum time in seconds that you allow each transfer to take'
complete -c curl -l metalink -d 'This option was previously used to specify a Metalink resource'
complete -c curl -l mptcp -d 'Enable the use of Multipath TCP (MPTCP) for connections'
complete -c curl -l version
complete -c curl -s n -l netrc -d 'Make curl scan the '
complete -c curl -l netrc-file -d 'Set the netrc file to use'
complete -c curl -l netrc-optional -d 'Similar to --netrc, but this option makes the '
complete -c curl -l verbose -l trace -l trace-ascii
...
complete -c curl -l no-clobber -d 'When used in conjunction with the --output, --remote-header-name,'
complete -c curl -l remote-name-all -d 'that already exist'
complete -c curl -l clobber -l remote-header-name -d specified
...
complete -c curl -l url -l user
complete -c curl -l out-null -d 'Discard all response output of a transfer silently'
complete -c curl -s o -l output -d 'Write output to the given file instead of stdout'
complete -c curl -s Z -l parallel -d 'Make curl perform all transfers in parallel as compared to the regular serial…'
complete -c curl -l parallel-immediate -d 'When doing parallel transfers, this option instructs curl to prefer opening u…'
complete -c curl -l parallel-max -d 'When asked to do parallel transfers, using --parallel, this option controls t…'
complete -c curl -l parallel-max-host -d 'When asked to do parallel transfers, using --parallel, this option controls t…'
complete -c curl -l pass -d '(TLS SCP SFTP) Passphrase for the private key used for SSH or TLS'
...
complete -c curl -l preproxy -d 'Use the specified SOCKS proxy before connecting to an HTTP or HTTPS --proxy'
...
complete -c curl -l proxy-ca-native -d '(TLS) Use the operating system\'s native CA store for certificate verification…'
...
complete -c curl -l proxy-http3 -d '(HTTP) Negotiate HTTP/3 with an HTTPS proxy'
...
complete -c curl -s p -l proxytunnel -d 'When an HTTP proxy is used --proxy, this option makes curl tunnel the traffic…'
...
complete -c curl -s J -d '(HTTP) Tell the --remote-name option to use the server-specified Content-Disp…'
complete -c curl -l no-remote-name
complete -c curl -s R -l remote-time -d 'Make curl attempt to figure out the timestamp of the remote file that is gett…'
complete -c curl -l remove-on-error -d 'Remove the output file if an error occurs'
...
complete -c curl -l retry-connrefused -d 'In addition to the other conditions, also consider ECONNREFUSED as a transien…'
...
complete -c curl -l sasl-authzid -d '(LDAP IMAP POP3 SMTP) Use this authorization identity (authzid), during SASL …'
complete -c curl -l sasl-ir -d '(LDAP IMAP POP3 SMTP) Enable initial response in SASL authentication'
complete -c curl -l service-name -d 'Set the service name for SPNEGO'
complete -c curl -s S -l show-error -d 'When used with --silent, it makes curl show an error message if it fails'
complete -c curl -s i -d '(HTTP FTP) Show response headers in the output'
complete -c curl -l no-styled-output -l styled-output -d '"--no-" prefix to disable it. )'
complete -c curl -l sigalgs -d '(TLS) Set specific signature algorithms to use during SSL session establishme…'
complete -c curl -s s -d 'Silent or quiet mode'
...
complete -c curl -l socks5-gssapi -d '(GSS/kerberos) Use GSS-API authentication when connecting to a SOCKS5 proxy'
complete -c curl -l socks5-gssapi-nec -d '(GSS/kerberos) As part of the GSS-API negotiation a protection mode is negoti…'
...
complete -c curl -l ssl-sessions -d '(TLS) **WARNING**: this option is experimental.  Do not use in production'
...
complete -c curl -s t -l telnet-option -d '(TELNET) Pass options to the telnet protocol.  Supported options are:'
complete -c curl -l tftp-blksize -d '(TFTP) Set the TFTP BLKSIZE option (must be 512 or larger)'
complete -c curl -l tftp-no-options -d '(TFTP) Do not send TFTP options requests'
...
complete -c curl -l tls-max -d '(TLS) Set the maximum allowed TLS version'
complete -c curl -l tls13-ciphers -d '(TLS) Set which cipher suites to use in the connection if it negotiates TLS 1'
...
complete -c curl -l 'tlsv1.0' -d '(TLS) Force curl to use TLS version 1'
complete -c curl -l 'tlsv1.1' -d '(TLS) Force curl to use TLS version 1'
complete -c curl -l 'tlsv1.2' -d '(TLS) Force curl to use TLS version 1'
complete -c curl -l 'tlsv1.3' -d '(TLS) Force curl to use TLS version 1'
...
complete -c curl -l unix-socket -d '(HTTP) Connect to the server through this Unix domain socket, instead of usin…'
complete -c curl -s T -l upload-file -d 'Upload the specified local file to the remote URL'
complete -c curl -l upload-flags -d '(IMAP) Specify additional behavior to apply to uploaded files'
complete -c curl -l url-query -d 'Add a piece of data, usually a name + value pair, to the end of the URL query…'
complete -c curl -s B -l use-ascii -d '(FTP LDAP TFTP) Enable ASCII transfer mode'
complete -c curl -s u -d 'Specify the username and password to use for server authentication'
complete -c curl -s A -l user-agent -d '(HTTP) Specify the User-Agent string to send to the HTTP server'
complete -c curl -s v -d 'Make curl output verbose information during the operation'
complete -c curl -l no-verbose -o vv -d '"-vv -v" is equivalent to a single -v'
complete -c curl -s V -d 'Display information about curl and the libcurl version it uses'
complete -c curl -l vlan-priority -d 'Set VLAN priority as defined in IEEE 802. 1Q'
complete -c curl -s w -l write-out -d 'Make curl display information on stdout after a completed transfer'
complete -c curl -l xattr -d 'Store metadata in the extended file attributes'
