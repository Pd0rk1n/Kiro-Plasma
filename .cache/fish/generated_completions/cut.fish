...
complete -c cut -s b -l bytes -d 'select only these byte positions'
complete -c cut -s c -l characters -d 'select only these character positions'
...
complete -c cut -s F -d 'like -f, but also implies -w and -O \' \''
complete -c cut -s n -l no-partial -d 'with -b, don\'t output partial multi-byte characters'
complete -c cut -s O -l output-delimiter -d 'use STRING as the output delimiter; the default is to use the input delimiter'
complete -c cut -s s -l only-delimited -d 'do not print lines not containing delimiters'
complete -c cut -s w -l whitespace-delimited -d 'use a run of blank characters as the field delimiter; with \'trimmed\', ignore …'
