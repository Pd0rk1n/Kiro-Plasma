...
complete -c systemd-nspawn -l cleanup -d 'Clean up left-over mounts and underlying mount points used by the container, …'
...
complete -c systemd-nspawn -l mstack -d 'Mount stack to mount the root directory for the container from'
...
complete -c systemd-nspawn -s u -l uid -d 'After transitioning into the container, change to the specified user defined …'
...
complete -c systemd-nspawn -l private-users-delegate -d 'Takes a non-negative integer'
...
complete -c systemd-nspawn -l restrict-address-families -d 'Restrict the socket address families accessible to the container'
...
complete -c systemd-nspawn -l forward-journal -d 'Forward the container\\*(Aqs journal to the host by starting systemd-journal-r…'
complete -c systemd-nspawn -l forward-journal-max-use -l forward-journal-keep-free -l forward-journal-max-file-size -l forward-journal-max-files -d 'These options configure the corresponding settings of systemd-journal-remote(…'
...
complete -c systemd-nspawn -l bind-user-shell -d 'When used with --bind-user=, includes the specified shell in the user records…'
complete -c systemd-nspawn -l bind-user-group -d 'When used with --bind-user=, includes the specified group as an auxiliary gro…'
...
complete -c systemd-nspawn -l background -d 'Change the terminal background color to the specified ANSI color as long as t…'
complete -c systemd-nspawn -l load-credential -l set-credential -d 'Pass a credential to the container'
complete -c systemd-nspawn -l system -l user -d 'Specify whether to run in system or user scope'
...
complete -c systemd-nspawn -l no-ask-password -d 'Do not query the user for authentication for privileged operations'
complete -c systemd-nspawn -o M/--machine
complete -c systemd-nspawn -o D/--directory
complete -c systemd-nspawn -o i/--image -d 'to determine the mounts to be cleaned up'
complete -c systemd-nspawn -l shift -d 'switch may be used to shift UID/GID ownership from or to the 0, foreign or sp…'
complete -c systemd-nspawn -l capabilities
complete -c systemd-nspawn -l use-host-config -d 'when using dnf <= 4'
