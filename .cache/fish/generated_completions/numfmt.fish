...
complete -c numfmt -l field -d 'replace the numbers in these input fields (default=1); see FIELDS below for d…'
complete -c numfmt -l format -d 'use printf style floating-point FORMAT; see FORMAT below for details'
complete -c numfmt -l from -d 'auto-scale input numbers to UNITs; default is \'none\'; see UNIT below for deta…'
complete -c numfmt -l from-unit -d 'specify the input unit size (instead of the default 1)'
complete -c numfmt -l grouping -d 'use locale-defined grouping of digits, e. g.  1,000,000'
...
complete -c numfmt -l suffix -d 'add SUFFIX to output numbers, and accept an optional SUFFIX in input numbers'
complete -c numfmt -l unit-separator -d 'insert SEP between number and unit on output, and accept an optional SEP in i…'
