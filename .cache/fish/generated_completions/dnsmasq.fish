...
complete -c dnsmasq -l log-malloc -d 'Enable logging of memory allocation and release, for debugging'
...
complete -c dnsmasq -l local-service -d 'Without parameter or with net parameter, restricts service to connected netwo…'
complete -c dnsmasq -s 2 -l no-dhcp-interface -d 'Do not provide DHCP, TFTP or router advertisement on the specified interface,…'
complete -c dnsmasq -l no-dhcpv4-interface -d 'Disable only IPv4 DHCP on the specified interface'
complete -c dnsmasq -l no-dhcpv6-interface -d 'Disable IPv6 DHCP and router advertisement on the specified interface'
...
complete -c dnsmasq -l filter-rr -d 'Remove records of the specified type(s) from answers'
complete -c dnsmasq -l cache-rr -d 'By default, dnsmasq caches A, AAAA, CNAME and SRV DNS record types'
...
complete -c dnsmasq -l do-0x20-encode -l no-0x20-encode -d 'Dnsmasq can scramble the case of letters in DNS queries it sends upstream as …'
...
complete -c dnsmasq -l dnssec-limits -d 'Override the default resource limits applied to DNSSEC validation'
...
complete -c dnsmasq -s O -l dhcp-option -d 'Send various extra options to DHCP clients'
complete -c dnsmasq -l dhcp-option-force -d 'This works in exactly the same way as  --dhcp-option except that the option w…'
complete -c dnsmasq -l dhcp-option-pxe -d 'A variant of --dhcp-option which defines options only sent in reply to PXE cl…'
complete -c dnsmasq -l dhcp-no-override -d '(IPv4 only) Disable re-use of the DHCP servername and filename fields as extr…'
complete -c dnsmasq -l leasequery -d 'Enable RFC 4388 leasequery'
complete -c dnsmasq -l dhcp-relay -d 'Configure dnsmasq to do DHCP relay'
complete -c dnsmasq -l dhcp-split-relay -d 'A usefully enhanced version of DHCPv4 relay'
...
complete -c dnsmasq -l dhcp-match -d 'Without <value>, set <tag> if the client sends a DHCP option of the given num…'
complete -c dnsmasq -l dhcp-name-match -d 'Set a <tag> if the given <name> is supplied by a DHCP client'
complete -c dnsmasq -l tag-if -d 'Perform boolean operations on tags'
complete -c dnsmasq -s J -l dhcp-ignore -d 'When all the given tags appear in the tag set, ignore the host and do not all…'
...
complete -c dnsmasq -l max-tcp-connections -d 'The maximum number of concurrent TCP connections'
