...
complete -c btrfs-filesystem -s L -l level -d 'Since kernel 6'
complete -c btrfs-filesystem -l nocomp -d 'Do not compress while defragmenting, uncompress extents if needed'
...
complete -c btrfs-filesystem -s p -l pagesize -d 'specify page size (k/m/g/e/p suffix), default is from running kernel, must be…'
...
complete -c btrfs-filesystem -l enqueue -d 'wait if there\'s another exclusive operation running, otherwise continue'
complete -c btrfs-filesystem -l offline -d 'resize an unmounted (offline) filesystem'
...
complete -c btrfs-filesystem -s z -l reset -d 'print stats and reset \'max_commit_ms\' (needs root) sync <path> Force a sync o…'
