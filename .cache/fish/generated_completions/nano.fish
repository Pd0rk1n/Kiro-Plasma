...
complete -c nano -s C -l backupdir -d 'Make and keep not just one backup file, but make and keep a uniquely numbered…'
complete -c nano -s D -l boldtext -d 'For the interface, use bold instead of reverse video'
complete -c nano -s E -l tabstospaces -d 'Convert each typed tab to spaces \\[em] to the number of spaces that a tab at …'
...
complete -c nano -s J -l guidestripe -d 'Draw a vertical stripe at the given column, to help judge the width of the te…'
complete -c nano -s K -l rawsequences -d 'Interpret escape sequences directly, instead of asking \\%ncurses to translate…'
...
complete -c nano -s N -l noconvert -d 'Disable automatic conversion of files from DOS format'
...
complete -c nano -s R -l restricted -d 'Read and write only the file (or files) specified on the command line'
complete -c nano -s S -l softwrap -d 'Display over multiple screen rows lines that exceed the screen\'s width'
complete -c nano -s T -l tabsize -d 'Set the size (width) of a tab to \\%number columns'
...
complete -c nano -s Y -l syntax -d 'Specify the name of the syntax highlighting to use from among the ones define…'
...
complete -c nano -s b -l breaklonglines -d 'Automatically hard-wrap the current line when it becomes overlong, that is: w…'
complete -c nano -s c -l constantshow -d 'Constantly report the cursor position on the status bar'
...
complete -c nano -s f -l rcfile -d 'Read only this file for setting nano\'s options, instead of reading both the s…'
...
complete -c nano -s o -l operatingdir -d 'Change to the given \\%directory, and allow reading and writing files only in …'
complete -c nano -s p -l preserve -d 'Preserve the XOFF and XON sequences (^S and ^Q) so that they are caught by th…'
complete -c nano -s q -l indicator -d 'Display a \\[lq]scrollbar\\[rq] on the righthand side of the edit window'
complete -c nano -s r -l fill -d 'Set the target width for justifying and automatic hard-wrapping at this \\%num…'
...
complete -c nano -s z -l listsyntaxes -d 'List the names of the available syntaxes and exit'
complete -c nano -l magic -d 'When neither the file\'s name nor its first line give a clue, try using \\%libm…'
complete -c nano -s @ -l colonparsing -d 'When a filename given on the command line ends in a colon plus digits and thi…'
...
complete -c nano -s 1 -l solosidescroll -d 'When the cursor approaches the right edge of the edit window, scroll only the…'
complete -c nano -l '%quickblank' -d '0. 8 seconds instead of after the default 1. 5 seconds'
complete -c nano -l '%constantshow' -d 'The state flags are displayed only when --%stateflags is used'
