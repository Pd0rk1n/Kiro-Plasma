...
complete -c appstreamcli -l limit -d 'flag.  Subsequent result pages can be requested using the'
complete -c appstreamcli -l start -d 'flag, which sets the index of the first review to fetch'
complete -c appstreamcli -l locale -d 'flag to select a different locale instead.  The'
complete -c appstreamcli -l server -d 'flag can be used to fetch reviews from a different ODRS-compatible server ins…'
