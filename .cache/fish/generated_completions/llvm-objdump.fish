...
complete -c llvm-objdump -l debug-indent -d 'Distance to indent the source-level variable or inlined function display, rel…'
complete -c llvm-objdump -l debug-inlined-funcs -d 'Print the locations of inlined functions alongside disassembly'
...
complete -c llvm-objdump -l offloading -d 'Display the content of the LLVM offloading sections and HIP offload bundles'
complete -c llvm-objdump -l prefix -d 'When disassembling with the --source option, prepend prefix to absolute paths'
complete -c llvm-objdump -l prefix-strip -d 'When disassembling with the --source option, strip out level initial director…'
...
complete -c llvm-objdump -l pretty-pgo-analysis-map -d 'When using --symbolize-operands with bb-address-map and pgo-analysis-map, pri…'
...
complete -c llvm-objdump -l disassemble-symbols -d 'Disassemble only the specified symbols.  Takes demangled symbol names when'
...
complete -c llvm-objdump -s x -l all-headers -d 'Display all available header information.  Equivalent to specifying'
complete -c llvm-objdump -o fbasic-block-address-map -d instead
complete -c llvm-objdump -l arch -d 'Specify the architecture to disassemble'
complete -c llvm-objdump -l archive-member-offsets -d 'Print the offset to each archive member for Mach-O archives (requires'
