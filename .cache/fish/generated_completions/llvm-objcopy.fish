...
complete -c llvm-objcopy -l discard-all -s x -d 'Remove most local symbols not referenced by relocations from the output'
...
complete -c llvm-objcopy -l extract-section -d 'Extract the specified section <section> into the file <file> as a seperate ob…'
...
complete -c llvm-objcopy -l change-section-address -l adjust-section-vma -d 'Change the address of sections that match <section> pattern to the specified …'
...
complete -c llvm-objcopy -l discard-locals -s X -d 'Remove local symbols starting with "'
...
complete -c llvm-objcopy -l remove-note -d 'Remove notes of integer type <type> and name <name> from SHT_NOTE sections th…'
...
complete -c llvm-objcopy -l split-dwo -d 'Equivalent to running llvm-objcopy with --extract-dwo and <dwo-file> as the o…'
complete -c llvm-objcopy -l strip-dwo
