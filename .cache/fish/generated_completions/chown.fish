...
complete -c chown -l reference -d 'use RFILE\'s ownership rather than specifying values'
complete -c chown -s R -l recursive -d 'operate on files and directories recursively '
complete -c chown -s H -d 'if a command line argument is a symlink to a directory, traverse it'
