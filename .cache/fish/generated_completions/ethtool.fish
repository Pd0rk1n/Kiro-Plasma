...
complete -c ethtool -l disable-netlink -d 'Do not use netlink and fall back to the ioctl interface if possible'
...
complete -c ethtool -l phy -d 'Target a PHY within the interface'
complete -c ethtool -s a -l show-pause -d 'Queries the specified Ethernet device for pause parameter information. RS 4'
complete -c ethtool -l src -d 'If the MAC Merge layer is supported, request a particular source of device st…'
...
complete -c ethtool -l groups -d 'Request groups of standard device statistics'
...
complete -c ethtool -l get-hwtimestamp-cfg -d 'Show the selected time stamping PTP hardware clock configuration'
complete -c ethtool -l set-hwtimestamp-cfg -d 'Sets the device\'s time stamping PTP hardware clock configuration. RS 4'
...
complete -c ethtool -l get-plca-cfg -d 'Show the current PLCA parameters for the given interface. RE'
complete -c ethtool -l set-plca-cfg -d 'Change the PLCA settings for the given interface. RS 4'
complete -c ethtool -l get-plca-status -d 'Show the current PLCA status for the given interface'
complete -c ethtool -l show-mm -d 'Show the MAC Merge layer state'
complete -c ethtool -l set-mm -d 'Set the MAC Merge layer parameters. RS 4'
complete -c ethtool -l show-pse -d 'Show the current Power Sourcing Equipment (PSE) status for the given interface'
complete -c ethtool -l set-pse -d 'Set Power Sourcing Equipment (PSE) parameters. RS 4'
complete -c ethtool -l show-mse -d 'Show PHY Mean Square Error (MSE) diagnostics. RS 4 '
complete -c ethtool -l flash-module-firmware -d 'Flash the transceiver module\'s firmware'
complete -c ethtool -l show-phys -d 'Show the PHY devices attached to an interface, and the way they link together'
