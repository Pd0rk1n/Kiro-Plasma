...
complete -c systemctl -s v -l verbose -d 'Display unit log output while executing unit operations'
complete -c systemctl -l no-warn -d 'Do not generate the warnings shown by default in the following cases: . sp'
...
complete -c systemctl -l kill-subgroup -d 'Takes a control group sub-path to send signals to, for use with the kill comm…'
...
complete -c systemctl -l kernel-cmdline -d 'When used with kexec, append the specified string to the kernel command line …'
...
complete -c systemctl -l marked -d 'enqueue-marked.  try-reload-or-restart PATTERN'
