...
complete -c ss -s Q -l no-queues -d 'Suppress sending and receiving queue columns'
...
complete -c ss -s p -l processes -d 'Show processes using sockets'
complete -c ss -s T -l threads -d 'Show threads using sockets.  Implies  -p '
...
complete -c ss -s b -l bpf -d 'Show socket classic BPF filters (only administrators are allowed to get this …'
...
complete -c ss -l bpf-maps -d 'Pretty-print all the BPF socket-local data entries for each socket'
complete -c ss -l bpf-map-id -d 'Pretty-print the BPF socket-local data entries for the requested map ID'
