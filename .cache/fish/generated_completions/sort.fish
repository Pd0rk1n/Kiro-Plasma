...
complete -c sort -s R -l random-sort -d 'shuffle, but group identical keys.   See also shuf(1)'
...
complete -c sort -s u -l unique -d 'output only the first of lines with equal keys; with -c, check for strict ord…'
