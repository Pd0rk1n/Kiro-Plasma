...
complete -c less -s A -l SEARCH-SKIP-SCREEN -d 'auses all forward searches (not just non-repeated searches) to start just aft…'
...
complete -c less -s d -l dumb -d 'The  "-d" option suppresses the error message normally displayed if the termi…'
...
complete -c less -s I -l IGNORE-CASE -d 'Like  "-i" "," but searches ignore case even if the pattern contains uppercas…'
...
complete -c less -o Ofilename -l LOG-FILE -d 'The  "-O" option is like  "-o," but it will overwrite an existing file withou…'
...
complete -c less -o Ps -d 'followed by a string changes the default (short) prompt to that string'
complete -c less -o Pm -d 'changes the medium . RB "(" "-m" ")" prompt'
complete -c less -o PM -d 'changes the long . RB "(" "-M" ")" prompt'
complete -c less -o Ph -d 'changes the prompt for the help screen'
complete -c less -s P -d 'changes the message printed by the = command'
complete -c less -o Pw -d 'changes the message printed while waiting for data (in the "F" command). RE'
...
complete -c less -s R -l RAW-CONTROL-CHARS -d 'Like  "-r" "," but only a limited set of escape  sequences are output in "raw…'
...
complete -c less -o ttag -l tag -d 'The  "-t" option, followed immediately by a TAG, will edit the file containin…'
...
complete -c less -s w -l hilite-unread -d 'Temporarily highlights the first "new" line after a movement of a full page'
complete -c less -s W -l HILITE-UNREAD -d 'Like  "-w" "," but temporarily highlights the first new line after any moveme…'
...
complete -c less -o '\\*~' -l tilde -d 'Normally lines after end of file are displayed as a single tilde (\\*~)'
complete -c less -s '#' -l shift -d 'Specifies the default number of positions to scroll horizontally in the RIGHT…'
complete -c less -l autosave -d 'Normally the history file is saved only when  less exits'
complete -c less -l cmd -d 'The specified string is taken to be an initial command to  less '
complete -c less -l emouse -d 'features is a comma-separated list of mouse features to be enabled, selected …'
complete -c less -l end-prompt -d 'Specifies a string which is printed immediately after the prompt is erased'
complete -c less -l exit-follow-on-close -d 'When using the "F" command on a pipe,  less will automatically stop waiting f…'
complete -c less -l file-size -d 'If  "--file-size" is specified,  less will determine the size of the file  im…'
complete -c less -l follow-name -d 'Normally, if the input file is renamed while an F command is executing,  less…'
complete -c less -l form-feed -d 'When scrolling forward or backward in the file, stop if a line beginning with…'
complete -c less -l header -d 'RS Sets the number of header lines and columns displayed on the screen'
complete -c less -l hilite-target -d 'Highlights the "target" line, as specified by the -j option'
complete -c less -l incsearch -d 'Subsequent search commands will be "incremental"; that is,  less will advance…'
complete -c less -l intr -d 'Use the character c instead of \\*^X to interrupt a read when the "Waiting for…'
complete -c less -l line-num-width -d 'Sets the minimum width of the line number field when the -N option is in effe…'
complete -c less -l match-shift -d 'When  "-S" is in effect, if a search match is not visible because it is shift…'
complete -c less -l modelines -d 'RS Before displaying a file,  less  will read the first n lines to try to fin…'
complete -c less -l mouse -d 'Enabling --mouse is the same as --emouse=vmove,click'
complete -c less -l MOUSE -d 'Like  "--mouse" with  "--rmouse" ". "'
complete -c less -l no-edit-warn -d 'Don\'t print a warning message when using the v command on a file which was op…'
...
complete -c less -l no-number-headers -d 'Header lines (defined via the  "--header" option) are not assigned line numbe…'
complete -c less -l no-paste -d 'If the terminal supports  xterm -compatible "bracketed paste", any text paste…'
...
complete -c less -l past-eof -d 'Normally scrolling commands (other than J, K, ESC-SPACE and ESC-b) stop scrol…'
complete -c less -l proc-backspace -d 'If set, backspaces are handled as if neither the  "-u" option  nor the  "-U" …'
complete -c less -l PROC-BACKSPACE -d 'If set, backspaces are handled as if the  "-U" option were set; that is backs…'
complete -c less -l proc-return -d 'If set, carriage returns are handled as if neither the  "-u" option  nor the …'
complete -c less -l PROC-RETURN -d 'If set, carriage returns are handled as if the  "-U" option were set; that is…'
complete -c less -l proc-tab -d 'If set, tabs are handled as if the  "-U" option were not set'
complete -c less -l PROC-TAB -d 'If set, tabs are handled as if the  "-U" option were set; that is tabs are tr…'
complete -c less -l redraw-on-quit -d 'When quitting, after sending the terminal deinitialization string, redraws th…'
complete -c less -l rmouse -d 'Reverses the normal direction of scrolling when mouse scrolling is enabled vi…'
...
complete -c less -l show-preproc-errors -d 'If a LESSOPEN preprocessor produces data,  then exits with a non-zero exit co…'
...
complete -c less -l wheel-lines -d 'Set the number of lines or columns to scroll when the mouse wheel is scrolled…'
complete -c less -l wordwrap -d 'When the  "-S" option is not in use, wrap each line at a space or tab if poss…'
complete -c less -s z -d 'below)'
complete -c less -s j -d 'options, which change this)'
complete -c less -s t -d 'option for more details about tags'
complete -c less -s b -d or
complete -c less -s h -d 'or a string value (such as'
...
complete -c less -l mou -d 'but not'
complete -c less -l mo -d 'since both'
complete -c less -l Quit-at-eof -d 'is equivalent to'
complete -c less -s k -d or
complete -c less -s D -d 'require a string to follow the option letter'
complete -c less -o Dug -d 'displays underlined text as green without underlining; the green color has re…'
complete -c less -o Du+g -d 'displays underlined text as both green and in underlined format'
complete -c less -o DNGk -d 'displays line numbers as bright green text on a black  background, and'
complete -c less -o DEbR -d 'displays error messages as blue text on a  bright red background'
complete -c less -o Da -d 'o For historical reasons, when a lowercase color selector letter  is followed…'
complete -c less -o h0 -d 'is implied. )'
complete -c less -s O -d 'option is like'
complete -c less -s o -d 'but it will overwrite an existing file without asking for confirmation'
complete -c less -o z-4 -d 'sets the scrolling window to 20 lines'
complete -c less -o cc -l quotes -d 'Changes the filename quoting character'
complete -c less -o '\\fP' -d 'Normally lines after end of file are displayed as a single tilde ()'
complete -c less -s p -d 'option is taken to be a less command rather than a search pattern'
