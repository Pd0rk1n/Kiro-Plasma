...
function yazi-1tb
    set mount /run/user/(id -u)/gvfs/smb-share:server=10.0.0.19,share=1tb
    if not test -d $mount
        echo "Mounting 1TB share..."
        gio mount smb://10.0.0.19/1tb
        sleep 1
    end
    yazi $mount
end
...
function yazi-4tb
    set mount /run/user/(id -u)/gvfs/smb-share:server=10.0.0.19,share=4tb
    if not test -d $mount
        echo "Mounting 4TB share..."
        gio mount smb://10.0.0.19/4tb
        sleep 1
    end
    yazi $mount
end
...
function yazi-6tb
    set mount /run/user/(id -u)/gvfs/smb-share:server=10.0.0.19,share=6tb
    if not test -d $mount
        echo "Mounting 6TB share..."
        gio mount smb://10.0.0.19/6tb
        sleep 1
    end
    yazi $mount
end
